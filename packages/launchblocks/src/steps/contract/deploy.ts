...
export const contractDeploy = defineStep({
  ...
  ui: {
    ...
    fields: [
      ...
      { key: "gas", label: "Gas limit", kind: "number" },
      { key: "initialHbar", label: "HBAR to send", kind: "amount", help: "Only for a payable constructor" },
      { key: "adminKey", label: "Admin key", kind: "boolean", help: "Without one the contract can never be changed" },
    ],
    outputs: [
